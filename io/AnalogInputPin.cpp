...
AnalogInputPin::AnalogInputPin ()
    : m_Channel(0), m_BufferSize(1), m_ValueSum(0), m_ValuesInBuffer(0)
{ }


AnalogInputPin::AnalogInputPin (uint8_t channel, uint8_t bufferSize)
    : m_Channel(channel), m_BufferSize(bufferSize), m_ValueSum(0), m_ValuesInBuffer(0)
{
    if (m_BufferSize > 16)
    {
        m_BufferSize = 16;
...
void AnalogInputPin::SelectChannel ()
{
    ...
    ADMUX &= ~(0x0f);
    ADMUX |= (m_Channel & 0x0f);
#endif

#if defined(__AVR__ATmega1280__) || defined(__AVR_ATmega2560__)
    ADMUX &= ~(0x07);
    ADMUX |= (m_Channel & 0x07);

    ADCSRB &= ~(1 << MUX5);
    ADCSRB |= (((m_Channel & 0x08) >> 3) << MUX5);
#endif
}
